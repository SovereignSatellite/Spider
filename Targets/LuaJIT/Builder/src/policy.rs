...
use luajit_foreign::{PointerStore, TableStore};
...
fn reuse_hint_of(node: &Node, port: u16) -> Option<Link> {
	if let Node::Foreign(foreign) = node {
		...
		if let Some(store) = any.downcast_ref::<PointerStore>() {
			return (port == PointerStore::STATE_PORT).then_some(store.reference);
		}

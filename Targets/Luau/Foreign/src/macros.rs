//! Declaration macros for Luau foreign operation nodes.

/// Declares a unary Luau foreign operation taking a single source link.
macro_rules! define_unary_operation {
	($name:ident, $identifier:literal) => {
		#[doc = concat!("The `", $identifier, "` Luau foreign operation.")]
		#[derive(Clone, Copy)]
		pub struct $name {
...
/// Declares a binary Luau foreign operation taking a left- and right-hand link.

...
use super::{ControlFlowGraph, instruction::Instruction};
...
fn format_instruction(instruction: Instruction, formatter: &mut Formatter<'_>) -> Result {
	match instruction {
...
pub struct Dot<'graph> {
	graph: &'graph ControlFlowGraph,
}

impl<'graph> Dot<'graph> {
	/// Creates a new DOT formatter for the given control flow graph.
	#[must_use]
	pub const fn new(graph: &'graph ControlFlowGraph) -> Self {
		Self { graph }
	}
	...
	fn fmt_nodes(&self, formatter: &mut Formatter<'_>) -> Result {
		...
		self.graph.block_ids().try_for_each(|id| {
			let instructions = self.graph.instructions(id);
			...
			instructions.iter().try_for_each(|&instruction| {
				format_instruction(instruction, formatter)?;
	...
	fn fmt_edges(&self, formatter: &mut Formatter<'_>) -> Result {
		...
		self.graph.block_ids().try_for_each(|id| {
			self.graph.successors(id).try_for_each(|successor| {

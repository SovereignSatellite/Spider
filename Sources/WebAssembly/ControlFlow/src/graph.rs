...
use super::instruction::{I32Constant, Instruction, LocalBranch, ReservedLocal};
...
impl BasicBlock {
	...
	pub fn replace_ids<Mapping: Fn(u16) -> u16>(&mut self, map: Mapping) {
...
/// Represent a directed graph of basic blocks whose structured branches and tail-controlled loops become topologically ordered after back-edge removal.
...
impl ControlFlowGraph {
	...
	pub fn branch_merge(&self, id: u16) -> u16 {
		let mut first_arm_entry = u16::MAX;
		let mut second_arm_entry = u16::MAX;

		for successor in self.successors_acyclic(id) {
			if successor < first_arm_entry {
				second_arm_entry = first_arm_entry;
				first_arm_entry = successor;
			} else if successor < second_arm_entry {
				second_arm_entry = successor;
			}
		}

		let first_arm_end = second_arm_entry - 1;
		let Some(end) = self.find_branch_end(first_arm_end) else {
			unreachable!()
		};
	...
	/// Returns `true` if the given block has an assignment to the reserved local.
	#[must_use]
	pub fn has_assignment(&self, id: u16, local: ReservedLocal) -> bool {
		matches!(self.instructions(id), &[Instruction::I32Constant(I32Constant { destination, .. })] if destination == local as u16)
	}
	...
	pub fn add_selection(&mut self, local: ReservedLocal) -> u16 {
	...
	pub fn add_assignment(&mut self, local: ReservedLocal, value: u16) -> u16 {
		let i32_constant = Instruction::I32Constant(I32Constant {
			destination: local as u16,
			data: value.into(),
		});

...
use super::rewrite::BranchRewriter;
use crate::ControlFlowGraph;

pub struct BranchNormalizer {
	rewriter: BranchRewriter,

	regions: Vec<(u16, u16)>,
}

impl BranchNormalizer {
	#[must_use]
	pub const fn new() -> Self {
		Self {
			rewriter: BranchRewriter::new(),
	...
	fn handle_region(&mut self, graph: &mut ControlFlowGraph, entry: u16, exit: u16) {
		let point = self.rewriter.run(graph, entry);

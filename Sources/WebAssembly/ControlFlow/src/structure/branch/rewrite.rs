...
use super::continuation::ContinuationFinder;
use crate::{ControlFlowGraph, instruction::ReservedLocal};

pub struct BranchRewriter {
	edges: Vec<(u16, u16)>,
	points: Vec<u16>,
	separators: Vec<usize>,

	continuation: ContinuationFinder,
}
...
impl BranchRewriter {
	pub const fn new() -> Self {
		Self {
			...
			continuation: ContinuationFinder::new(),
		}
	}
	...
	fn has_assignment_in_branch(&self, graph: &ControlFlowGraph) -> bool {
		self.points.iter().any(|&id| {
			graph.predecessors(id).any(|predecessor_id| {
				graph.has_assignment(predecessor_id, ReservedLocal::BranchSelector)
	...
	fn exclude_last_assignments(&mut self, graph: &ControlFlowGraph) {
		let excluded = self.points.iter().flat_map(|&id| {
			graph
				.predecessors(id)
				.filter(|&id| graph.has_assignment(id, ReservedLocal::BranchSelector))
				.map(|id| {
					let mut predecessors = graph.predecessors(id);

					if let Some(predecessor_id) = predecessors.next()
						&& predecessors.next().is_none()
						&& graph.has_assignment(predecessor_id, ReservedLocal::RepeatSelector)
					{
						predecessor_id
					} else {
						id
		...
		self.continuation.set_excluded(excluded);
	}
	...
	fn find_continuations(&mut self, graph: &ControlFlowGraph, entry: u16, id: u16) {
		...
		if predecessors.next().is_some() && predecessors.next().is_some() {
			...
			self.continuation.run(graph, id);
			self.continuation.edges_into(graph, &mut self.edges);
		}
	...
	fn set_new_continuation(&mut self, graph: &mut ControlFlowGraph) -> u16 {
		let selection = graph.add_selection(ReservedLocal::BranchSelector);

		for (from, to) in &mut self.edges {
			let point = self.points.binary_search(to).unwrap().try_into().unwrap();
			let assignment = graph.add_assignment(ReservedLocal::BranchSelector, point);
	...
	pub fn run(&mut self, graph: &mut ControlFlowGraph, entry: u16) -> u16 {
		self.continuation.set_excluded(iter::empty());

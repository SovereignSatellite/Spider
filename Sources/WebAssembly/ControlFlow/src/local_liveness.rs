...
use super::{
	...
	instruction::{
		...
		MemoryLoad, MemorySize, MemoryStore, NumberBinaryOperation, NumberCompareOperation,
		NumberNarrow, NumberTransmuteToInteger, NumberTruncateToInteger, NumberUnaryOperation,
		NumberWiden, RefFunction, RefIsNull, RefNull, ReservedLocal, TableCopy, TableFill,
...
impl LiveLocals {
	...
	pub fn get_union<Identifiers: IntoIterator<Item = u16>>(
		&self,
		ids: Identifiers,
		union: &mut Vec<u16>,
	) {
...
impl LocalLiveness {
	/// Create a local-liveness analysis.
	#[must_use]
	pub const fn new() -> Self {
		Self {
			reads: Set::new(),
	...
	fn read_local(&mut self, local: u16) {
		self.slot_count = self.slot_count.max(local + 1);
	...
	fn read_successors_in(&mut self, locals: &LiveLocals, graph: &ControlFlowGraph, id: u16) {
		for successor in graph.successors_acyclic(id) {
			self.read_boundary_or_empty(locals, successor);
		}
	}

	fn read_boundary_or_empty(&mut self, locals: &LiveLocals, id: u16) {
		let live = locals.get_or_empty(id).iter().copied().map(usize::from);

		self.reads.extend(live);
	}

	fn handle_start(&mut self, locals: &mut LiveLocals, graph: &ControlFlowGraph, id: u16) {
	...
	fn handle_end(&mut self, locals: &mut LiveLocals, graph: &ControlFlowGraph, id: u16) {
	...
	fn handle_all(&mut self, locals: &mut LiveLocals, graph: &ControlFlowGraph, result_count: u16) {
		self.reads.clear();

		for result in 0..result_count {
			self.read_local(result + ReservedLocal::COUNT);
		}
...
impl Default for LocalLiveness {
	fn default() -> Self {
		Self::new()
	}
}

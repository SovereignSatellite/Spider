...
use web_assembly_control_flow::instruction::{Reference, ReferenceType};
...
impl ModuleBindings {
	...
	pub fn collect_dependencies_into<References: IntoIterator<Item = Reference>>(
		&self,
		...
		target.extend(
			references
				.into_iter()
				.map(|reference| self.get_dependency(reference)),
		);
	}
}

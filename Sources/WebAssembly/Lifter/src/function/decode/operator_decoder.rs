...
use web_assembly_control_flow::{ControlFlowGraph, instruction::Location};

use super::{
	DECODER_SCRATCH, LOCAL_BASE,
	block_builder::BlockBuilder,
	operand_stack::{ControlLevel, ControlLevelKind, OperandStack, PendingJump},
};
use crate::module::TypeRegistry;

pub struct OperatorDecoder {
	block_builder: BlockBuilder,
	operand_stack: OperandStack,
}

impl OperatorDecoder {
	...
	fn add_single_target_table_branch(&mut self) -> u16 {
		self.operand_stack.pull_local();

		self.block_builder.add_basic_block(1)
	}

	fn add_boolean_branch(&mut self) -> u16 {
		let selector = self.operand_stack.pull_local();
		...
		self.block_builder.add_local_branch(DECODER_SCRATCH, 2)
	}
	...
	fn add_indexed_table_branch(&mut self, branch_count: usize) -> u16 {
		...
		self.block_builder.add_local_branch(selector, branch_count)
	}
	...
	fn handle_unreachable(&mut self) {
		let id = self.block_builder.add_unreachable();

		self.operand_stack.set_top(u16::MAX);
		self.operand_stack.jump_to_level(id, 0, 0);
	}

	fn handle_block(&mut self, types: &TypeRegistry, block_type: BlockType) {
		self.operand_stack
			.push_level(types, block_type, ControlLevelKind::Forward);
	}

	fn handle_loop(&mut self, types: &TypeRegistry, block_type: BlockType) {
		let id = self.block_builder.add_basic_block(1) + 1;

		self.operand_stack
			.push_level(types, block_type, ControlLevelKind::Loop { entry: id });
	}

	fn handle_if(&mut self, types: &TypeRegistry, block_type: BlockType) {
		let id = self.add_boolean_branch();

		self.operand_stack
			.push_level(types, block_type, ControlLevelKind::Forward);
		self.operand_stack.jump_to_depth(id, 0, 0);
	}
	...
	fn handle_else(&mut self) {
		let ControlLevel {
			...
			pending_jumps,
			..
		} = self.operand_stack.peek_level_mut();
		...
		let PendingJump {
			...
			successor_index,
			..
		} = pending_jumps.swap_remove(0);
		let then_exit = self.block_builder.add_basic_block(1);

		self.operand_stack.jump_to_depth(then_exit, 0, 0);

		let else_entry = then_exit + 1;
		self.block_builder
			.set_jump_destination(source_block, successor_index, else_entry);

		self.operand_stack.set_top(top);
	}

	fn handle_end(&mut self) {
		let top = self.operand_stack.get_top();
		let level = self.operand_stack.pull_level();

		self.block_builder.handle_level(level, top);
	}

	fn handle_br(&mut self, relative_depth: u32) {
		let id = self.block_builder.add_basic_block(1);

		self.operand_stack.jump_to_depth(id, 0, relative_depth);
	}

	fn handle_br_if(&mut self, relative_depth: u32) {
		let id = self.add_boolean_branch();

		self.operand_stack.jump_to_depth(id, 1, relative_depth);
	}

	fn handle_br_table(&mut self, table: &BrTable<'_>) {
		let target_count = table.len().try_into().unwrap();
		let id = self.add_table_branch(target_count + 1);

		self.operand_stack
			.jump_to_depth(id, target_count, table.default());

		for (branch, relative_depth) in table.targets().map(Result::unwrap).enumerate() {
			self.operand_stack.jump_to_depth(id, branch, relative_depth);
		}
	}
	...
	fn handle_call(&mut self, function: u32, kind: &FuncType) {
		...
		let (destinations, sources) = self.operand_stack.load_function_type(kind);

		self.block_builder
			.add_ref_function(DECODER_SCRATCH, function);
		self.block_builder
			.add_call(destinations, sources, DECODER_SCRATCH);
	}

	fn handle_call_indirect(&mut self, table: u32, kind: &FuncType) {
		let function = Location {
			reference: table.try_into().unwrap(),
			offset: self.operand_stack.pull_local(),
		};
		let (destinations, sources) = self.operand_stack.load_function_type(kind);

		self.block_builder.add_table_get(DECODER_SCRATCH, function);
		self.block_builder
			.add_call(destinations, sources, DECODER_SCRATCH);
	}

	const fn handle_drop(&mut self) {
		let _item = self.operand_stack.pull_local();
	}

	fn handle_select(&mut self) {
		let (destination, on_true, on_false, condition) =
			self.operand_stack.load_ternary_operation();
	...
	fn handle_local_tee(&mut self, local: u32) {
		let destination = LOCAL_BASE + u16::try_from(local).unwrap();
		let source = self.operand_stack.peek_local();
	...
	fn handle_global_get(&mut self, global: u32) {
		...
		self.block_builder.add_global_get(destination, source);
	}
	...
	fn handle_global_set(&mut self, global: u32) {
		...
		self.block_builder.add_global_set(destination, source);
	}

	fn handle_load(&mut self, memory_argument: MemArg, kind: LoadType) {
		let (destination, offset) = self.operand_stack.load_unary_operation();
		let source = Location {
		...
		self.block_builder
			.add_memory_load(destination, source, kind);
	}
	...
	fn handle_memory_size(&mut self, memory: u32) {
		...
		self.block_builder
			.add_paged_memory_size(destination, memory);
	}

	fn handle_memory_grow(&mut self, memory: u32) {
		let memory = memory.try_into().unwrap();
		let (destination, size) = self.operand_stack.load_unary_operation();
	...
	fn handle_i32_const(&mut self, data: i32) {
		...
		self.block_builder.add_i32_constant(destination, data);
	}

	fn handle_i64_const(&mut self, data: i64) {
		let destination = self.operand_stack.push_local();

		self.block_builder.add_i64_constant(destination, data);
	}
	...
	fn handle_f32_const(&mut self, data: Ieee32) {
		...
		self.block_builder
			.add_f32_constant(destination, data.into());
	}
	...
	fn handle_f64_const(&mut self, data: Ieee64) {
		...
		self.block_builder
			.add_f64_constant(destination, data.into());
	}

	fn handle_i32_equals_zero(&mut self) {
		let (destination, lhs) = self.operand_stack.load_unary_operation();
	...
	fn handle_i64_equals_zero(&mut self) {
		...
		self.block_builder.add_i64_constant(DECODER_SCRATCH, 0);
		self.block_builder.add_integer_compare_operation(
			destination,
			lhs,
			DECODER_SCRATCH,
			integer::Type::I64,
	...
	fn handle_integer_unary_operation(
		...
		let (destination, source) = self.operand_stack.load_unary_operation();
	...
	fn handle_integer_compare_operation(
		...
		let (destination, lhs, rhs) = self.operand_stack.load_binary_operation();
	...
	fn handle_reversed_integer_compare_operation(
		...
		let (destination, rhs, lhs) = self.operand_stack.load_binary_operation();
	...
	fn handle_integer_narrow(&mut self) {
		...
		self.block_builder.add_integer_narrow(destination, source);
	}
	...
	fn handle_number_truncate(
		...
		self.block_builder.add_number_truncate_to_integer(
			destination,
			source,
			is_signed,
			is_saturating,
	...
	fn handle_integer_widen(&mut self, is_signed: bool) {
		...
		self.block_builder.add_integer_widen(destination, source);
	...
	fn handle_number_narrow(&mut self) {
		...
		self.block_builder.add_number_narrow(destination, source);
	}

	fn handle_number_widen(&mut self) {
		let (destination, source) = self.operand_stack.load_unary_operation();

		self.block_builder.add_number_widen(destination, source);
	}
	...
	fn handle_memory_init(&mut self, memory: u32, data: u32) {
		let size = self.operand_stack.pull_local();
		let source = Location {
		...
		self.block_builder
			.add_memory_init(destination, source, size);
	}

	fn handle_data_drop(&mut self, data: u32) {
		let data = data.try_into().unwrap();

		self.block_builder.add_data_drop(data);
	}
	...
	fn handle_memory_fill(&mut self, memory: u32) {
		let size = self.operand_stack.pull_local();
		let byte = self.operand_stack.pull_local();
		let destination = Location {
			reference: memory.try_into().unwrap(),
			offset: self.operand_stack.pull_local(),
		};

		self.block_builder.add_memory_fill(destination, byte, size);
	}
	...
	fn handle_table_init(&mut self, table: u32, elements: u32) {
		...
		self.block_builder.add_table_init(destination, source, size);
	}

	fn handle_elements_drop(&mut self, elements: u32) {
		let elements = elements.try_into().unwrap();

		self.block_builder.add_elements_drop(elements);
	}
	...
	fn handle_ref_null(&mut self) {
		...
		self.block_builder.add_ref_null(destination);
	}

	fn handle_ref_is_null(&mut self) {
		let (destination, source) = self.operand_stack.load_unary_operation();

		self.block_builder.add_ref_is_null(destination, source);
	}
	...
	fn handle_ref_function(&mut self, function: u32) {
		...
		self.block_builder.add_ref_function(destination, function);
	}
	...
	fn handle_table_fill(&mut self, table: u32) {
		...
		self.block_builder.add_table_fill(destination, source, size);
	}
	...
	fn handle_table_get(&mut self, table: u32) {
		...
		self.block_builder.add_table_get(destination, source);
	}
	...
	fn handle_table_set(&mut self, table: u32) {
		...
		self.block_builder.add_table_set(destination, source);
	}

	fn handle_table_grow(&mut self, table: u32) {
		let table = table.try_into().unwrap();
		let (destination, initializer, size) = self.operand_stack.load_binary_operation();
	...
	fn handle_table_size(&mut self, table: u32) {
		...
		self.block_builder.add_table_size(destination, table);
	}
	...
	fn handle_operator(&mut self, types: &TypeRegistry, operator: &Operator<'_>) {
		match *operator {
	...
	pub fn run(
		...
		self.block_builder.add_basic_block(0);

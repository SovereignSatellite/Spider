...
pub fn print(root: &Arc<Mutex<Function>>, out: &mut dyn Write) {
	...
	print_library(&function, out).expect("failed to write the Luau runtime library");
	print_function(&function, out).expect("failed to write the compiled Luau source");
}

pub fn print_runtime(out: &mut dyn Write) {
	print_full_library(out).expect("failed to write the Luau runtime library");
}
